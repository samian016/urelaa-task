...
const find = ({ table, key = {} }) => new Promise((resolve, reject) => {
  ...
  delete key?.query;
  // May break
  resolve(table[method](key, options)[noPaginate ? 'populate' : 'then'](populate))
...
const findOne = async ({ table, key = {} }) => new Promise((resolve, reject) => {
  ...
  table.findOne(key).populate(key.populate)
    .then(res => resolve(res))
    .catch(e => reject(e));
});
 ...
 */
const save = async (data, populate = null) => {
  if (!populate) return await data.save();
  await data.save();
  return await data.populate(populate);
};
...
export { find, findOne, create, remove, update, save, removeAll, populate };